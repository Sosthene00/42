...
void	get_values(t_ctx *c, t_sprt *s)
{
	...
	s->height = abs((int)(c->screen.height / (s->transform_y)));
	...
	s->width = abs((int)(c->screen.height / s->transform_y));
...
void	draw_stripes(t_ctx *c, t_ima *texture, t_sprt *s)
{
	...
	while (++stripe < s->draw_end_x)
	{
		...
		y = s->draw_start_y;
		while (y < s->draw_end_y && s->transform_y > 0 &&
			s->transform_y < c->ray.zbuffer[stripe])
		{
			d = y * 256 - c->screen.height * 128 + s->height * 128;
