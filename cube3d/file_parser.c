...
int get_xy_value(char *line, ctx *c)
{
    char **val;
...
int update_data(char *line, ctx *c)
{
...
int parse_file(char *map_file, ctx *c)
{
    ...
    {
        if (c->complete < 3 && ft_isalpha(line[0]))
        {
            if (update_data(line, c) == 2)
                break;
            c->complete++;
        }
        else if (c->complete == 3 && *line)
        {
